...
with open(os.path.join('src', 'sqlalchemy_litesync', 'constants.py'), 'rt') as f:
    exec(f.read())
...
setup_params = dict(
    name="sqlalchemy_litesync",
    version=__version__,
    description="SQLAlchemy dialect for Litesync",
    author=__author__,
    ...
    entry_points={
        "sqlalchemy.dialects":
            ["sqlite.litesync = sqlalchemy_litesync.litesync:dialect"]
    },
    license=__license__,
)
